...
class MatehanAction(object):
    ...
    def init_variable(self, *args, **kwargs):
        ...
        # TAM THOI: test detect 2 dot tai vi tri docking (xem SEND_DOCKING_HUB)
        self.dock_test_mode = True          # False = tra ve chu trinh docking that
        self.max_dock_test = 10             # so lan detect THANH CONG moi dot
        self.dock_test_rotate_deg = 45.0    # doi thanh -45.0 neu xoay nguoc chieu
        self.dock_test_settle = 1.0         # thoi gian dung im cho on dinh (s)
        self.dock_test_phase = 0
        self.dock_test_ok = [0, 0]
        self.dock_test_try = [0, 0]
        self.dock_test_t0 = 0.0
        self.dock_test_target_yaw = None
        # tf
    ...
    def execute_cb(self, goal):
        ...
        while not rospy.is_shutdown():
            ...
            # State: SEND_GOTO_TEMP_POSE
                ...
                if not self.compute_goals_from_mirror():
                    ...
                    if self.count_rotate != 2:
                        ...
                        rospy.logwarn_throttle(5, "[MATEHAN] Mirror not found after search, moving to error state.")
                        rospy.sleep(0.5)
                        _state_when_error = _state
            ...
            # State: SEND_DOCKING_HUB
                ...
                # ==========================================================
                # TAM THOI: test detect tai vi tri docking, 2 dot.
                #   dot 1: huong hien tai (nhu binh thuong)
                #   dot 2: sau khi xoay DOCK_TEST_ROTATE_DEG de lidar sau
                #          huong ve guong
                # Moi dot: dung im -> cho on dinh -> dem du 10 lan detect
                # THANH CONG. Xong ca 2 dot thi vao DETECT_MIRROR_ERROR.
                # Dat self.dock_test_mode = False de tra ve chu trinh that.
                # ==========================================================
                if self.dock_test_mode:
                    mirror_pose_target = self.calculate_pose_offset(
                        0.4,
                        hub_pose_x,
                        hub_pose_y,
                        atan2(
                    ...
                    # ---- dung banh, giu robot dung im ----
                    if self.dock_test_phase in (0, 1, 2, 5, 6, 7):
                        self.vel.linear.x = 0.0
                        self.vel.angular.z = 0.0
                        self.cmd_vel_pub.publish(self.vel)

                    # ---- phase 0/5: bat detect roi bat dau dem 1 s on dinh ----
                    if self.dock_test_phase in (0, 5):
                        dot = 1 if self.dock_test_phase == 0 else 2
                        # use_scan_merge = False -> dung dung lidar nhu docking that
                        self.send_request_get_mirror(
                            mirror_pose_target,
                            True,
                            orientation_hub_to_waiting_pose
                        )
                        self.dock_test_t0 = rospy.get_time()
                        rospy.logwarn(
                            "[DOCK TEST] DOT {}: bat detect, cho {:.1f}s cho on dinh".format(
                                dot, self.dock_test_settle
                            )
                        )
                        self.dock_test_phase += 1
                        r.sleep()
                        continue

                    # ---- phase 1/6: cho on dinh ----
                    if self.dock_test_phase in (1, 6):
                        if rospy.get_time() - self.dock_test_t0 >= self.dock_test_settle:
                            self.dock_test_phase += 1
                        r.sleep()
                        continue

                    # ---- phase 2/7: dem 10 lan detect thanh cong ----
                    if self.dock_test_phase in (2, 7):
                        dot = 1 if self.dock_test_phase == 2 else 2
                        idx = 0 if dot == 1 else 1
                        self.dock_test_try[idx] += 1
                        if self.compute_goals_from_mirror():
                            self.dock_test_ok[idx] += 1
                            rospy.logwarn(
                                "[DOCK TEST] DOT {} OK {}/{} (try {}) mirror odom "
                                "x:{:.4f} y:{:.4f} yaw:{:.2f} deg".format(
                                    dot,
                                    self.dock_test_ok[idx],
                                    self.max_dock_test,
                                    self.dock_test_try[idx],
                                    self.trans_mirror[0],
                                    self.trans_mirror[1],
                                    degrees(euler_from_quaternion(self.rot_mirror)[2]),
                                )
                            )
                        else:
                            rospy.logwarn(
                                "[DOCK TEST] DOT {} FAIL (try {}) - success {}/{}".format(
                                    dot,
                                    self.dock_test_try[idx],
                                    self.dock_test_ok[idx],
                                    self.max_dock_test,
                        ...
                        if self.dock_test_ok[idx] >= self.max_dock_test:
                            self.send_request_get_mirror(
                                mirror_pose_target,
                                False,
                                orientation_hub_to_waiting_pose
                            ...
                            if dot == 1:
                                rospy.logwarn(
                                    "[DOCK TEST] DOT 1 XONG: {} success / {} try -> xoay {:.1f} deg".format(
                                        self.dock_test_ok[0],
                                        self.dock_test_try[0],
                                        self.dock_test_rotate_deg,
                                    )
                                )
                                self.dock_test_phase = 3
                            else:
                                rospy.logwarn(
                                    "[DOCK TEST] XONG CA 2 DOT | dot1 {}/{} try | dot2 {}/{} try "
                                    "-> DETECT_MIRROR_ERROR".format(
                                        self.dock_test_ok[0], self.dock_test_try[0],
                                        self.dock_test_ok[1], self.dock_test_try[1],
                                    )
                                )
                                _state_when_error = _state
                                _state = MainState.DETECT_MIRROR_ERROR
                    ...
                    # ---- phase 3: chot goc dich cho lan xoay ----
                    if self.dock_test_phase == 3:
                        self.dock_test_target_yaw = self.normalize_angle(
                            self.robot_pose_angle
                            + math.radians(self.dock_test_rotate_deg)
                        )
                        rospy.logwarn(
                            "[DOCK TEST] xoay tu {:.2f} deg -> {:.2f} deg".format(
                                degrees(self.robot_pose_angle),
                                degrees(self.dock_test_target_yaw),
                            )
                        )
                        self.dock_test_phase = 4
                        r.sleep()
                        continue

                    # ---- phase 4: xoay tai cho (dung sai rieng, khong dung
                    #      rotate_from_init_angle vi ham do dung sai 0.3 rad) ----
                    if self.dock_test_phase == 4:
                        angle_err = self.normalize_angle(
                            self.dock_test_target_yaw - self.robot_pose_angle
                        )
                        if abs(angle_err) < 0.01:
                            ...
                            self.cmd_vel_pub.publish(self.vel)
                            rospy.logwarn(
                                "[DOCK TEST] xoay xong, sai so {:.3f} deg".format(
                                    degrees(angle_err)
                                )
                            )
                            self.dock_test_phase = 5
                        else:
                            self.vel.linear.x = 0.0
                            if angle_err > 0:
                                self.vel.angular.z = np.clip(0.5 * angle_err, 0.15, 0.35)
                            else:
                                self.vel.angular.z = np.clip(0.5 * angle_err, -0.35, -0.15)
                            self.cmd_vel_pub.publish(self.vel)
                            rospy.loginfo_throttle(
                                2, "[DOCK TEST] dang xoay, con {:.2f} deg".format(
                                    degrees(angle_err))
                            )
                        r.sleep()
                        continue
            ...
            # .##.....##..#######.....###....####.##....##..######...#######.########.##.....##.##.....##..#######..##.....##
                ...
                if self._asm.reset_error_request:
                    ...
                    self.count_rotate = 0
                    self.search_base_angle = None
                    # TAM THOI: bat dau lai loat test detect tai docking
                    self.dock_test_phase = 0
                    self.dock_test_ok = [0, 0]
                    self.dock_test_try = [0, 0]
                    self.dock_test_target_yaw = None
