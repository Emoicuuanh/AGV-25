...
class PassboxAction(object):
    ...
    def init_variable(self, *args, **kwargs):
        ...
        self.plc_ip = rospy.get_param("~plc_ip", "192.86.11.192")
        self.plc_port = rospy.get_param("~plc_port", 5000)
        rospy.loginfo("PLC config: {}:{} (will connect when mission starts)".format(self.plc_ip, self.plc_port))
        # Khong ket noi ngay khi khoi dong - se ket noi khi co mission vao execute_cb
        ...
        self.length_passbox = 1.15
        self.choose_config_docking = False #True khi docking lift false khi docking hub

        # Tìm gương bằng cách xoay tại chỗ (chỉ dùng khi docking bàn nâng)
        self.count_rotate = 0
    ...
    def read_error_plc():
        result = modbus_tcp_passbox.read_slave_2(1, error_code_1, 1)
        if not result:
            print("read_error_plc: Failed to read register {}".format(error_code_1))
            return None
        raw_value = result[0]  # giá trị 16-bit (0–65535)
        print("read_error_plc: raw register {} = {}".format(error_code_1, raw_value))
        ...
        try:
            ...
            with open(hub_cfg_file) as j:
                ...
                if "mirror_offsets_lift" in hub_dict:
                    ...
                    self.mirror_offsets_lift = {
                        "hub": {"x_offset": -0.42, "y_offset": 0.0},
                        "waiting": {"x_offset": 2.0, "y_offset": 0.0},
                        "temp": {"x_offset": 2.0, "y_offset": 0.0}
                    }
            # Waiting path config
...
################################################################################
        ...
        # Hướng mặt gương trong frame map (field HubService/orientation).
        # mirror_detect dùng góc này để loại các cặp gương lệch hướng, nên phải
        # gửi đúng hướng từ điểm đích ra phía robot tiếp cận.
        orientation_hub_to_inside_pose = atan2(
            inside_pose_y - hub_pose_y, inside_pose_x - hub_pose_x
        )
        orientation_hub_to_waiting_pose = atan2(
            waiting_pose_y - hub_pose_y, waiting_pose_x - hub_pose_x
        )
        if lift_pose_x is not None and lift_pose_y is not None:
            orientation_hub_to_lift_pose = atan2(
                lift_pose_y - hub_pose_y, lift_pose_x - hub_pose_x
            )
            orientation_lift_to_waiting_pose = atan2(
                waiting_pose_y - lift_pose_y, waiting_pose_x - lift_pose_x
            )
            # Hướng lý tưởng của robot khi mặt về phía bàn nâng, dùng làm góc gốc
            # khi xoay tìm gương ở SEND_ROTATE_INIT_LIFT
            orientation_waiting_to_lift_pose = atan2(
                lift_pose_y - waiting_pose_y, lift_pose_x - waiting_pose_x
            )
        else:
            orientation_hub_to_lift_pose = orientation_hub_to_waiting_pose
            orientation_lift_to_waiting_pose = orientation_hub_to_waiting_pose
            orientation_waiting_to_lift_pose = self.normalize_angle(
                orientation_hub_to_waiting_pose + pi
            )
        ...
        # Reset trạng thái quét tìm gương của bàn nâng cho mission mới
        self.count_rotate = 0
        self.search_base_angle = None
        # Kết nối tới PLC khi có mission vào execute_cb
        rospy.logwarn("Mission received - connecting to PLC {}:{}".format(self.plc_ip, self.plc_port))
        # Thu lai vai lan: lan bat tay TCP dau tien hay bi timeout
        # roi lan thu 2 lai thanh cong -> khong nen vao NETWORK_ERROR ngay.
        modbus_tcp_passbox.connect_retry(self.plc_ip, self.plc_port, attempts=3)
        if self.check_connected():
            rospy.sleep(1)
...
############################3
                ...
                self.send_request_get_mirror(
                    ...
                    self.length_passbox,
                    True,
                    orientation_lift_to_waiting_pose,
                    use_scan_merge=True,
                )
                ...
                if not self.compute_goals_from_mirror({
                    ...
                    # Không thấy gương: xoay tại chỗ ±45 deg quanh góc gốc để quét lại
                    if self.search_base_angle is None:
                        # Chọn góc gốc dựa trên việc đầu hay đuôi đang hướng về bàn nâng
                        ideal_yaw = orientation_waiting_to_lift_pose
                        diff_forward = abs(
                            self.normalize_angle(self.robot_pose_angle - ideal_yaw)
                        )
                        diff_backward = abs(
                            self.normalize_angle(
                                self.robot_pose_angle - (ideal_yaw + pi)
                            )
                        )
                        ...
                        if diff_forward < diff_backward:
                            ...
                            self.search_base_angle = self.normalize_angle(
                                ideal_yaw + pi
                            )
                        rospy.logwarn(
                            "Search base angle for detect mirror: {:.2f} deg".format(
                                degrees(self.search_base_angle)
                            )
                        )

                    if self.count_rotate != 2:
                        _state = MainState.SEND_ROTATE_INIT_LIFT
                    else:
                        rospy.logwarn_throttle(
                            5,
                            "[PASSBOX LIFT] Mirror not found after search, moving to error state.",
                        )
                        rospy.sleep(0.5)
            ...
            # State: 
            # Xoay tại chỗ ±45 deg quanh góc gốc để quét tìm gương bàn nâng.
            # Chỉ dùng cho luồng docking bàn nâng (SEND_GOTO_LIFT_TEMP_POSE).
            elif _state == MainState.SEND_ROTATE_INIT_LIFT:
                if self.count_rotate == 0:
                    rospy.logwarn_throttle(2, "XOAY LAN 1 ")
                    target_yaw = self.normalize_angle(
                        self.search_base_angle + math.radians(45)
                    )
                else:
                    rospy.logwarn_throttle(2, "XOAY LAN 2 ")
                    target_yaw = self.normalize_angle(
                        self.search_base_angle - math.radians(45)
                    )
                angle_err = self.normalize_angle(
                    target_yaw - self.robot_pose_angle
                )
                if self.rotate_from_init_angle(angle_err):
                    rospy.loginfo(
                        "Initial rotation finished. Proceeding to lift temp pose."
                    )
                    self.count_rotate += 1
                    ...
                    rospy.loginfo_throttle(
                        5,
                        "Initial rotation in progress... Error: {:.3f} rad".format(
                            angle_err
                        ),
                    )
                # if modbus_tcp_passbox.read_slave(1,lift_table_state, 1)[0] == 2:
                #     # Dừng xoay trước khi vào state lỗi
                #     self.rotate_from_init_angle(0.0)
                #     self._asm.reset_flag()
                #     self.moving_control_run_pause_pub.publish(
                #         StringStamped(stamp=rospy.Time.now(), data="PAUSE")
                #     )
            ...
            # #######.................................
                ...
                if floor_equal:
                    self.send_request_get_mirror(
                        ...
                        self.length_hub,
                        True,
                        orientation_hub_to_inside_pose,
                        use_scan_merge=True,
                    )
                    rospy.sleep(1)
                    if not self.compute_goals_from_mirror({
...
#########################3 Rotate LIFT DOCKING ##################################
            ...
            #.....#.#######....#.....##.##..#######.#.....#.#....#.........#######.#.....#.
                ...
                try:
                    if self.check_connected():
                        ...
                        rospy.logerr(
                            "Connect to passbox error. Retry connect ..."
                        )
                        # Mất kết nối thật sự -> phải mở lại kết nối,
                        # check_connected() không tự reconnect được.
                        if modbus_tcp_passbox.connect(
                            self.plc_ip, self.plc_port
                        ):
                            rospy.logwarn("Reconnect to passbox success")
                        rospy.sleep(1)
                except Exception as e:
                    rospy.logerr(e)
                ...
                if self.check_connected() and self.mode_robot == "AUTO":
                    ...
                    # Vong lap chinh vua bi chan trong connect()/sleep suot
                    # thoi gian mat ket noi PLC, khong ai doc feedback cua
                    # moving control. Watchdog 5s dung wall-clock nen se bao
                    # "/moving control disconnected!" gia ngay khi quay lai.
                    # Reset moc thoi gian de cho moving control 5s tu dau.
                    self.last_moving_control_fb = rospy.get_time()
            ...
            # State: GOING_TO_OUT_OF_HUB
                ...
                if self.moving_control_result == GoalStatus.SUCCEEDED:
                    if floor_equal:
                        modbus_tcp_passbox.write_slave(1,lift_close_door_clean_side,1)
                        _state = MainState.DONE
                        rospy.sleep(2)
                        modbus_tcp_passbox.write_slave(1,lift_close_door_clean_side,0)
                    else:
            ...
            # ........................................................
                ...
                modbus_tcp_passbox.write_slave(1,lift_open_door_clean_side,0)
                self.moving_control_client.send_goal(
            ...
            # State: DONE
                ...
                self.dynamic_reconfig_movebase(self.vel_move_base,  publish_safety=True, stop_center_qr=False)
                if goal_type == PLACE:
                    success = True
                    break
                elif goal_type == PICK:
    ...
    def rotate_from_init_angle(self, angle):
        """Xoay tại chỗ để tìm gương (dùng cho SEND_ROTATE_INIT_LIFT).

        Vận tốc xoay lớn hơn rotate_reach_angle/rotate_to_goal vì đây là bước
        quét tìm gương, không phải bước canh chỉnh chính xác.
        """
        error_angle = angle
        ...
        if error_angle > 0:
            self.vel.angular.z = np.clip(0.1 * error_angle, 0.3, 0.5)
        else:
            self.vel.angular.z = np.clip(0.1 * error_angle, -0.5, -0.3)
        self.vel.linear.x = 0.0
        self.cmd_vel_pub.publish(self.vel)
        return False
    ...
    def send_request_get_mirror(
        self, pose_target, length, enable_detect, orientation, use_scan_merge=False
    ):
        """Gọi hub_service để bật/tắt detect gương.

        Args:
            orientation (float): hướng mặt gương trong frame map (rad).
                mirror_detect dùng để loại các cặp gương lệch hướng.
        """
        ...
        try:
            ...
            rospy.loginfo(
                f"hub_service call success: {res.success}, "
                f"orientation: {degrees(orientation):.2f} deg, "
                f"use_scan_merge: {use_scan_merge}"
            )
            return res.success
        except rospy.ServiceException as e:
